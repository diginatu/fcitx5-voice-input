...
- Runtime flow: a configurable hotkey (default `F12`) starts listening, audio is captured via PulseAudio into an in-memory WAV buffer (`lastRecording_`), the buffer is POSTed as `multipart/form-data` to a Whisper-compatible or OpenAI-compatible (speaches/OpenAI) HTTP endpoint via libcurl on a detached worker thread (default `http://localhost:9000/asr?output=txt&encode=false`; the multipart field and extra parts depend on the selected API format), and the resulting transcript is dispatched back to the Fcitx event loop and committed to the active input context.
